...
# %%
print(f"Number of outfits: {len(allOutfits):,}")
# Example outfit
print("Outfit keys:")
...
# %%
train_data = pd.read_csv("raw/data/train.csv", header=None)
train_data = np.array(train_data)
train_users = np.unique(train_data[:, 0])
print("Number of train data: {:,}".format(len(train_data)))
valid_data = pd.read_csv("raw/data/valid.csv", header=None)
valid_data = np.array(valid_data)
valid_users = np.unique(valid_data[:, 0])
print("Number of valid data: {:,}".format(len(valid_data)))
test_data = pd.read_csv("raw/data/test.csv", header=None)
test_data = np.array(test_data)
test_users = np.unique(test_data[:, 0])
print("Number of test data: {:,}".format(len(test_data)))
all_users = np.unique(np.concatenate([train_users, valid_users, test_users]))
# all users should appear in all split
assert set(all_users) == set(train_users) == set(valid_users) == set(test_users)
print("Number of users: {:,}".format(len(all_users)))

# %%
num_outfits_user_train = [len(train_data[train_data[:, 0] == user]) for user in all_users]
print("Number of outfits per user: min: {}, max: {}".format(min(num_outfits_user_train), max(num_outfits_user_train)))
num_outfits_user_valid = [len(valid_data[valid_data[:, 0] == user]) for user in all_users]  
print("Number of outfits per user: min: {}, max: {}".format(min(num_outfits_user_valid), max(num_outfits_user_valid)))
num_outfits_user_test = [len(test_data[test_data[:, 0] == user]) for user in all_users]
print("Number of outfits per user: min: {}, max: {}".format(min(num_outfits_user_test), max(num_outfits_user_test)))
